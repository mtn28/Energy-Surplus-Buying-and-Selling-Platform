...
export default function IniciarSessao() {
    ...
    useEffect(() => {
        if (dataUtilizadores.length > 0) {
            const user = dataUtilizadores.find(data => data.email === username);
            if (user) {
    ...
    async function HandleLogin(event) {
        ...
        try {
            ...
            if (res === "" || res === false) {
                ...
                const user = dataUtilizadores.find((data) => data.email === username);
                if (user) {
                    await idLocal(username); // Aguarda a resolução da promessa
                    if (user.ligacaoUsers[0].tipoutilizadoreCode === "V") {
                        window.location.href = `/dashboard_vendedor/${user.iduser}`;
                    } else if (user.ligacaoUsers[0].tipoutilizadoreCode === "C") {
                        window.location.href = `/dashboard_comprador/${user.iduser}`;
