...
export default function DashboardAdmin() {
  ...
  function ComprasSite() {
    ...
    return dataContratos.map((data, contractIndex) => {
      const contrato = data;
      let nomeC = "";
      let nomeV = "";

      dataEstatisticas.forEach((dataN) => {
        if (dataN.iduser === contrato.utilizadorIdV) {
          nomeV = dataN.nomeuser;
        } else if (dataN.iduser === contrato.utilizadorIdC) {
          nomeC = dataN.nomeuser;
        }
      });
      return dataOfertas.map((dataO, offerIndex) => {
        if (dataO.idoferta === data.ofertaIdoferta) {
          return (
            <React.Fragment key={contractIndex + '-' + offerIndex}>
              <tr>
                <td>
                ...
                <td>
                  <button
                    ...
                    onClick={() => setSelectedContractIndex(contractIndex)}
                  >
              ...
              <Modal
                show={selectedContractIndex === contractIndex}
